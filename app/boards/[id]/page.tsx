...
import { ArrowLeft, Rocket, MapPin, Sparkles, Share2 } from 'lucide-react';
          ...
          {boardItems.length > 0 && (
            <>
              <button
                ...
                className="flex items-center gap-1 text-amber-600 hover:text-amber-700 text-xs font-semibold px-2 py-1 rounded-lg hover:bg-amber-50 transition-colors flex-shrink-0"
                title="Post-trip timeline"
              >
                <Sparkles size={13} />
                Timeline
              </button>
              <button
                ...
                onClick={async () => {
                  if (!board) return;
                  const { buildSharePackage, shareBoard } = await import('@/lib/shareBoard');
                  const pkg = await buildSharePackage(board);
                  await shareBoard(pkg, board.name);
                }}
                className="flex items-center gap-1 text-indigo-600 hover:text-indigo-700 text-xs font-semibold px-2 py-1 rounded-lg hover:bg-indigo-50 transition-colors flex-shrink-0"
              >
                <Share2 size={13} />
                Share
              </button>
