...
export default function BoardsPage() {
  ...
  type BoardFilter = 'all' | 'planning' | 'visited';
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 dark:bg-gray-950 md:pl-16">
      ...
      {/* Filter tabs */}
      {!reorderMode && boards.length > 0 && (
        <div className="bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 px-4 pb-2 flex gap-2">
          {(['all', 'planning', 'visited'] as const).map((f) => (
            <button
              key={f}
              type="button"
            ...
            >
              {f === 'all' ? 'All' : f === 'planning' ? '🗺 Planning' : '✓ Visited'}
            </button>
          ))}
        </div>
      )}
        ...
        {boardsLoading ? (
          ...
          (() => {
            const visibleBoards = boards.filter((b) =>
              ...
              boardFilter === 'visited' ? !!b.completedAt :
              !b.completedAt
            );
            return visibleBoards.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-40 text-center px-6">
                <p className="text-sm text-gray-400">
                  {boardFilter === 'visited' ? 'No visited boards yet.' : 'No boards in planning.'}
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
