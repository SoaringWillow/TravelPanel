...
// Platforms that commonly block scraping — Vision is especially valuable for these.
const VISION_PRIORITY_PLATFORMS = new Set(['xiaohongshu', 'wechat', 'douyin']);
...
export async function POST(req: NextRequest) {
  ...
  // Use Vision when: caller provided a screenshot, OR scraping returned nothing
  // for a platform known to block bots (Xiaohongshu, WeChat, Douyin).
  const scrapingBlocked =
    VISION_PRIORITY_PLATFORMS.has(platform) &&
    !page?.textContent?.trim();
  const useVision = !!(imageBase64 && (scrapingBlocked || imageBase64));
...
Never return an empty substance array for a real travel post.`;
  ...
  try {
    if (useVision && imageBase64) {
      // Vision path: include the screenshot so Claude can read text/maps in the image
