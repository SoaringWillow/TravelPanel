...
**Implemented**: `browser-extension/` — Manifest V3 extension with popup UI, context menus (right-click page/link), settings page (configurable app URL), PNG icon generator. Opens `/share?url=…` flow in TravelPanel to reuse existing clip UI. Safari: run `xcrun safari-web-extension-converter browser-extension/`.
