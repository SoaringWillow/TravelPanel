...
export default function InboxCard({
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 space-y-3">
        ...
        <div className="flex items-center justify-between pt-1">
          {isRetrying ? (
            ...
            <span className="text-xs text-amber-600 dark:text-amber-400 font-medium">⚠ Analysis failed</span>
          )}
          <div className="flex items-center gap-1.5">
            <a href={item.url} target="_blank" rel="noopener noreferrer"
              className="p-1.5 text-gray-400 hover:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-lg transition-colors"
            ...
            {!isRetrying && onRetry && (
              <button type="button" onClick={() => onRetry(item.id, item.url)}
                className="text-xs font-medium px-3 py-1.5 rounded-lg border border-amber-300 dark:border-amber-700 text-amber-700 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-900/30 transition-colors">
                Retry
              </button>
            )}
            <button type="button" onClick={() => onDelete(item.id)}
  ...
  // First substance item for inline preview
  const firstTip = item.substance?.[0];
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
      ...
      <div className="p-4">
        ...
        {/* First substance item as inline tip preview */}
        {firstTip && (
          <p className="text-xs text-gray-500 dark:text-gray-400 italic line-clamp-2 mb-2 leading-relaxed border-l-2 border-indigo-200 dark:border-indigo-700 pl-2">
            {firstTip.content}
          </p>
        )}

        {/* Description (show only if no substance preview) */}
        {!firstTip && item.description && (
          <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 mb-2 leading-relaxed">
            {item.description}
          </p>
        )}

        {/* Meta row: location count + substance count */}
        {(item.locations.length > 0 || item.activities.length > 0 || (item.substance?.length ?? 0) > 0) && (
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            {item.locations.length > 0 && (
              <span className="inline-flex items-center gap-0.5 text-xs text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30 px-2 py-0.5 rounded-full font-medium">
                <MapPin size={9} />
                {item.locations.length} place{item.locations.length !== 1 ? 's' : ''}
              </span>
            )}
            {(item.substance?.length ?? 0) > 0 && (
              <span className="inline-flex items-center gap-0.5 text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 px-2 py-0.5 rounded-full font-medium">
        ...
        {item.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-3">
            {item.tags.slice(0, 3).map((tag) => (
              <span key={tag} className="bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 text-xs px-2 py-0.5 rounded-full">
                #{tag}
              </span>
            ))}
          </div>
