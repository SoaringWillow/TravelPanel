...
async function checkPendingAppGroupShare(router: ReturnType<typeof useRouter>) {
  try {
    ...
    const [{ value: title }, { value: screenshot }] = await Promise.all([
      Preferences.get({ key: 'pendingShareTitle' }),
      Preferences.get({ key: 'pendingShareScreenshot' }),
    ]);

    await Promise.all([
      Preferences.remove({ key: 'pendingShareURL' }),
      Preferences.remove({ key: 'pendingShareTitle' }),
      Preferences.remove({ key: 'pendingShareScreenshot' }),
    ]);

    // Stash the screenshot for the share page to read via sessionStorage
    if (screenshot) {
      try { sessionStorage.setItem('travelpanel_pending_screenshot', screenshot); } catch { /* ignore */ }
    }
      ...
      try {
        ...
        const listener = await App.addListener('appUrlOpen', async ({ url }) => {
          try {
            ...
            // If the extension flagged an image, read it from App Group storage now
            if (hasImage) {
              try {
                const { Preferences } = await import('@capacitor/preferences');
                const { value: screenshot } = await Preferences.get({ key: 'pendingShareScreenshot' });
                if (screenshot) {
                  await Preferences.remove({ key: 'pendingShareScreenshot' });
                  sessionStorage.setItem('travelpanel_pending_screenshot', screenshot);
                }
              } catch { /* Preferences unavailable — continue without image */ }
            }
