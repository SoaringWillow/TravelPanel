...
async function checkPendingAppGroupShare(router: ReturnType<typeof useRouter>) {
  try {
    ...
    // Image data is too large for URL params — ferry it via sessionStorage
    if (imageData) {
      try {
        ...
        // sessionStorage unavailable — proceed without image
      }
    }
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // Image data (if any) is always written to App Group and read separately.
        const listener = await App.addListener('appUrlOpen', async ({ url }) => {
          try {
            ...
            if (shareUrl) {
              // Opportunistically pull image data from App Group (written by Share Extension)
              try {
                const { Preferences } = await import('@capacitor/preferences');
                const { value: imageData }     = await Preferences.get({ key: 'pendingShareImageData' });
                const { value: imageMimeType } = await Preferences.get({ key: 'pendingShareImageMimeType' });
