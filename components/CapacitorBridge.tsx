...
async function checkPendingAppGroupShare(router: ReturnType<typeof useRouter>) {
  try {
    ...
    // Relay the image to the share page via sessionStorage (too large for URL params)
    if (image) {
      try { sessionStorage.setItem('pendingShareImage', image); } catch { /* noop */ }
    }
      ...
      try {
        ...
        const listener = await App.addListener('appUrlOpen', async ({ url }) => {
          try {
            ...
            if (shareUrl) {
              // If the extension stored an image (e.g. Xiaohongshu), relay to sessionStorage
              if (hasImage) {
                try {
                  const { Preferences } = await import('@capacitor/preferences');
                  const { value: image } = await Preferences.get({ key: 'pendingShareImage' });
