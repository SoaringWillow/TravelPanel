...
// Reads a pending share URL (and optional screenshot) stored by the iOS Share
// Extension via App Groups. The App Group suite name must match ShareViewController.swift.
async function checkPendingAppGroupShare(router: ReturnType<typeof useRouter>) {
  try {
    ...
    const { value: imageB64 } = await Preferences.get({ key: 'pendingShareImage' });
    ...
    // Stash the screenshot so the share page can pick it up after navigation
    if (imageB64) {
      try { sessionStorage.setItem('pendingShareImage', imageB64); } catch { /* ignore */ }
    }
      ...
      try {
        ...
        const listener = await App.addListener('appUrlOpen', async ({ url }) => {
          try {
            ...
            if (shareUrl) {
              // Check for a screenshot stashed in App Group by the Share Extension
              try {
                const { Preferences } = await import('@capacitor/preferences');
                const { value: imageB64 } = await Preferences.get({ key: 'pendingShareImage' });
                if (imageB64) {
                  await Preferences.remove({ key: 'pendingShareImage' });
                  sessionStorage.setItem('pendingShareImage', imageB64);
                }
              } catch { /* no-op outside native context */ }
