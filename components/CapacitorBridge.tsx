...
async function checkPendingAppGroupShare(router: ReturnType<typeof useRouter>) {
  try {
    ...
    const { value: imageMime } = await Preferences.get({ key: 'pendingShareImageType' });
    ...
    // Store image in sessionStorage so the share page can pick it up without
    // a URL-length-limited query parameter.
    if (image) {
      try {
        sessionStorage.setItem('pendingShareImage', image);
        sessionStorage.setItem('pendingShareImageType', imageMime ?? 'image/jpeg');
      } catch {
        // sessionStorage not available — image will be skipped
      }
    }
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        const listener = await App.addListener('appUrlOpen', async ({ url: deepLink }) => {
          try {
            // Normalise the custom scheme to a parseable HTTPS URL
            const parsed = new URL(deepLink.replace(/^[a-z][a-z0-9+\-.]*:\/\//i, 'https://app/'));
            ...
            // When the Share Extension flagged that it stored an image in App Group,
            // retrieve it now (before the router push so it's ready when /share mounts).
            if (hasImage) {
              try {
                const { Preferences } = await import('@capacitor/preferences');
                const { value: image }     = await Preferences.get({ key: 'pendingShareImage' });
                ...
                // Preferences not available — proceed without image
              }
            }
