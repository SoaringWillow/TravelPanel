...
export const PENDING_IMAGE_KEY = '__travelPanelPendingImage';
...
async function checkPendingAppGroupShare(router: ReturnType<typeof useRouter>) {
  try {
    ...
    // Stash image in sessionStorage so the /share page can consume it without
    // polluting the URL (images can be 100+ KB base64).
    if (image) {
      try { sessionStorage.setItem(PENDING_IMAGE_KEY, image); } catch { /* quota */ }
    }
      ...
      try {
        ...
        const listener = await App.addListener('appUrlOpen', async ({ url }) => {
          try {
            ...
            // When hasImage=1, the Share Extension wrote an image to App Group.
            // Read it now and stash in sessionStorage before routing.
            if (hasImage) {
              try {
                const { Preferences } = await import('@capacitor/preferences');
                const { value: image } = await Preferences.get({ key: 'pendingShareImage' });
