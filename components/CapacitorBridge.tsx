...
// Reads a pending share URL + optional image stored by the iOS Share Extension via App Groups.
...
async function checkPendingAppGroupShare(router: ReturnType<typeof useRouter>) {
  try {
    ...
    if (imageBase64) {
      try { sessionStorage.setItem('tp_pending_image', imageBase64); } catch { /* storage full */ }
    }
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // When hasImage=1, the Share Extension already wrote the JPEG base64 to App Group.
        const listener = await App.addListener('appUrlOpen', async ({ url }) => {
          try {
            ...
            // If the extension flagged an image, read it from App Group before navigating
            if (hasImage) {
              try {
                const { Preferences } = await import('@capacitor/preferences');
