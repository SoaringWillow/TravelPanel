...
async function checkPendingAppGroupShare(router: ReturnType<typeof useRouter>) {
  try {
    ...
    // Store image in sessionStorage so the /share page can pass it to enrichItem
    if (imageBase64) sessionStorage.setItem('pendingClipImage', imageBase64);
      ...
      try {
        ...
        const listener = await App.addListener('appUrlOpen', async ({ url }) => {
          try {
            ...
            if (shareUrl) {
              // Check for a screenshot the Share Extension may have saved alongside the URL
              try {
                const { Preferences } = await import('@capacitor/preferences');
                const { value: imageBase64 } = await Preferences.get({ key: 'pendingShareImageBase64' });
                if (imageBase64) {
                  sessionStorage.setItem('pendingClipImage', imageBase64);
                  await Preferences.remove({ key: 'pendingShareImageBase64' });
                }
              } catch {
                // Preferences not available — no image, proceed anyway
              }
