...
// Reads a pending share (URL + optional image) from the iOS Share Extension via App Groups.
async function checkPendingAppGroupShare(router: ReturnType<typeof useRouter>) {
  try {
    ...
    // Store image for the share page to pick up during enrichment
    if (image) sessionStorage.setItem('pendingShareImage', image);
      ...
      try {
        ...
        const listener = await App.addListener('appUrlOpen', async ({ url }) => {
          try {
            ...
            if (shareUrl) {
              // Pull any pending screenshot written to App Group by the Share Extension
              try {
                const { Preferences } = await import('@capacitor/preferences');
                ...
                // No Preferences plugin — continue without image
              }
