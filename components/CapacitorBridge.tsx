...
async function checkPendingAppGroupShare(router: ReturnType<typeof useRouter>) {
  try {
    ...
    // putting megabytes of base64 into a URL parameter.
    if (imageBase64) {
      sessionStorage.setItem('pendingShareImage', imageBase64);
      sessionStorage.setItem('pendingShareImageType', imageMediaType ?? 'image/jpeg');
    }
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // When hasImage=1 is present, the image is in App Group Preferences.
        const listener = await App.addListener('appUrlOpen', async ({ url }) => {
          try {
            ...
            // If the extension flagged a pending image, read it from App Group now.
            if (hasImage) {
              try {
                const { Preferences } = await import('@capacitor/preferences');
                const { value: imageBase64 }    = await Preferences.get({ key: 'pendingShareImage' });
                ...
                // Preferences unavailable — image extraction skipped
              }
            }
