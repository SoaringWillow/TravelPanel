...
async function checkPendingAppGroupShare(router: ReturnType<typeof useRouter>) {
  try {
    ...
    // Stash image in sessionStorage so the /share page can pick it up
    if (imageBase64) {
      try { sessionStorage.setItem('pendingShareImage', imageBase64); } catch { /* ignore */ }
    }
      ...
      try {
        ...
        const listener = await App.addListener('appUrlOpen', async ({ url }) => {
          try {
            ...
            if (shareUrl) {
              if (hasImage) {
                // Image was saved to App Group by the Share Extension — read and relay it
                try {
                  const { Preferences } = await import('@capacitor/preferences');
                  const { value: imageBase64 } = await Preferences.get({ key: 'pendingShareImageBase64' });
                  if (imageBase64) {
