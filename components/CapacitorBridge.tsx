      ...
      try {
        ...
        const listener = await App.addListener('appUrlOpen', async ({ url }) => {
          try {
            ...
            if (shareUrl) {
              // When hasImage=1 the Share Extension stored a JPEG base64 in App Group
              // Preferences. Move it into sessionStorage so the share page can read it.
              if (hasImage) {
                try {
                  const { Preferences } = await import('@capacitor/preferences');
                  const { value: imageBase64 } = await Preferences.get({ key: 'pendingShareImage' });
                  ...
                  // Preferences not available — continue without image
                }
              }
