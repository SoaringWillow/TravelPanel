...
async function checkPendingAppGroupShare(router: ReturnType<typeof useRouter>) {
  try {
    ...
    // imageData from App Group may be large — only forward if reasonable size
    if (imageData && imageData.length < 100_000) qs.set('imageData', imageData);
        ...
        const listener = await App.addListener('appUrlOpen', ({ url }) => {
          try {
            ...
            if (shareUrl) {
              ...
              // Pass thumbnail image from Xiaohongshu/WeChat share payload for Vision extraction
              if (imageData && imageData.length < 100_000) qs.set('imageData', imageData);
              router.push(`/share?${qs.toString()}`);
            }
          } catch {
