...
// Reads a pending share (URL + optional screenshot) stored by the iOS Share Extension.
// Screenshot is written to pendingShareImage in App Group preferences as base64 JPEG.
// We store it in sessionStorage before routing so the /share page can pick it up.
async function checkPendingAppGroupShare(router: ReturnType<typeof useRouter>) {
  try {
    ...
    // Store image in sessionStorage so the /share page can consume it
    if (imageBase64) {
      try {
        sessionStorage.setItem('pendingShareImage', imageBase64);
      } catch {
        // sessionStorage full or unavailable — vision will be skipped gracefully
      }
    }
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // When hasImage=1, the screenshot is already in pendingShareImage (App Group),
        // which checkPendingAppGroupShare will bridge into sessionStorage.
        const listener = await App.addListener('appUrlOpen', async ({ url }) => {
          try {
            ...
            if (shareUrl) {
              // If image is pending in App Group, bridge it to sessionStorage first
              if (hasImage) {
                try {
                  const { Preferences } = await import('@capacitor/preferences');
                  const { value: imageBase64 } = await Preferences.get({ key: 'pendingShareImage' });
                  await Preferences.remove({ key: 'pendingShareImage' });
                  if (imageBase64) sessionStorage.setItem('pendingShareImage', imageBase64);
                } catch {
                  // Preferences unavailable — vision skipped gracefully
                }
              }
