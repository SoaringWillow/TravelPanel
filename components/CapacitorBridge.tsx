...
async function checkPendingAppGroupShare(router: ReturnType<typeof useRouter>) {
  try {
    ...
    // Relay image to share page via sessionStorage (can't use URL params for binary data)
    if (imageBase64) {
      try {
        sessionStorage.setItem('pendingShareImage', JSON.stringify({
          base64: imageBase64,
      ...
      try {
        ...
        const listener = await App.addListener('appUrlOpen', async ({ url }) => {
          try {
            ...
            // When the Share Extension signals it stored an image, relay it to sessionStorage
            if (hasImage) {
              try {
                const { Preferences } = await import('@capacitor/preferences');
                const { value: imageBase64 } = await Preferences.get({ key: 'pendingShareImageBase64' });
