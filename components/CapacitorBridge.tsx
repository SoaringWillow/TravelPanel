...
// so that @capacitor/preferences reads from the same UserDefaults suite.
async function checkPendingAppGroupShare(router: ReturnType<typeof useRouter>) {
  try {
    ...
    if (imageBase64) {
      try { sessionStorage.setItem('__tpShareImage', imageBase64); } catch {}
    }
      ...
      try {
        ...
        const listener = await App.addListener('appUrlOpen', async ({ url }) => {
          try {
            ...
            if (shareUrl) {
              // If the extension stored a screenshot, retrieve it before navigating
              if (hasImage) {
                try {
                  const { Preferences } = await import('@capacitor/preferences');
